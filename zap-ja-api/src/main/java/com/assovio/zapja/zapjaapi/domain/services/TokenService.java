...
public class TokenService {
    ...
    //					.withExpiresAt(this.genExpirationDate(8)).sign(algorithm);
    //
    //			return token;
    //
    ...
    public String generateToken(Usuario usuario) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(secret);
            String token = JWT.create().withIssuer("auth-api").withSubject(usuario.getLogin()).sign(algorithm);
            return token;
        } catch (JWTCreationException exception) {
            throw new RuntimeException("Erro ao gerar token", exception);
        }
    ...
    private Instant genExpirationDate(Integer horas) {
